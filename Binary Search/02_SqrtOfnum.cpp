...
int mySqrt(int x)
{
    ...
    while (low <= high)
    {
        ...
        if ((long)x == mid * mid)
        { // if square of mid is equal to x than mid is the answer

            return (int)mid;
        }
        if ((mid * mid) < (long)x)
        { // if square of mid is less than x than we will store mid in count variable and low will be equal to mid
            count = mid;
            low = mid + 1;
        }
        else
        { // else if square of mid is greater than mid that we will reduce high to mid
            high = mid - 1;
        }
    }
...
int main()
{
    ...
    int res = mySqrt(num);
    cout << "The Square Root of the given number is " << res;
    return 0;
}